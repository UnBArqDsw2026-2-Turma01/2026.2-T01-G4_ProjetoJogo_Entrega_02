...
Representar a estrutura estática do domínio do jogo em nível conceitual, evidenciando as classes que sustentam os serviços já identificados no Diagrama de Componentes e o comportamento descrito no Diagrama de Atividades, sem detalhar aspectos de implementação.
...
O diagrama foi construído em [draw.io](https://app.diagrams.net/), seguindo o mesmo padrão visual (fundo preto, linhas e texto em branco, fonte monoespaçada) adotado pelos demais diagramas estáticos e dinâmicos da SubEquipe_01. A fonte editável (`.drawio`) e o SVG exportado estão em [`assets/classes/`](assets/classes/classes.drawio ':ignore').
...
|--------|-------------------|------------|
| `Personagem` («abstract») | Generaliza atributos de combate comuns a `Jogador` e `Inimigo` (vida, ataque, defesa). | RF06, RF07, RF16, RF17 |
| `Jogador` | Entidade controlada pelo usuário: move-se, coleta elementos, combina-os, usa itens e consulta seu progresso. | RF01–RF05, RF16 |
| `Inimigo` | Participante do combate com fraqueza elemental usada no cálculo de dano. | RF04, RF06, RF13, RF17 |
| `Inventario` | Mantém os elementos químicos e itens do jogador; realiza o componente Inventário. | RF04, RF18, RF19 |
| `ElementoQuimico` | Recurso coletável, insumo de combinações e magias. | RF03, RF05, RF09, RF18 |
| `Item` | Objeto criado por combinação fora de combate ou vendido por NPCs. | RF04, RF05, RF08, RF14 |
| `Combinacao` | Mistura de elementos fora de combate que consome ingredientes e produz um item. | RF05 (ata inicial) |
| `Magia` | Efeito determinado pela combinação de elementos coletados, aplicado em combate. | RF05, RF09 (Subgrupo 03) |
| `DiarioDoAventureiro` | Registra combinações e magias descobertas, evitando duplicidade; realiza o componente Diário. | RF06, RF08, RF10–RF12 |
| `LivroColecionavel` | Objeto espalhado pelo mundo que, ao ser coletado, é registrado no Diário. | RF10–RF12 |
| `Mundo` | Agrega as áreas exploráveis pelo jogador. | RF01, RF02 |
| `Area` | Unidade do mundo classificada por `TipoArea`. | RF01, RF02, RF04 |
| `TipoArea` («enumeration») | Segura, não segura de mundo aberto, não segura de mundo não aberto. | RF02 |
| `NPC` | Vende itens e oferece sidequests. | RF14, RF15 |
| `Sidequest` | Missão secundária oferecida por um NPC. | RF15 |
| `Combate` | Coordena o confronto por turnos, calcula dano e determina vitória/derrota; realiza o componente Combate em Turnos. | RF04, RF06, RF07, RF09, RF13, RF16, RF17 |
| `Partida` | Sessão de jogo que coordena o fluxo geral e o salvamento; realiza o componente Controle da Partida. | RF03 |

</div>

<p align="center">Tabela 1: Classes identificadas e requisitos associados. Fonte: requisitos do Subgrupo 03 e artefatos da SubEquipe_01, organizados por Cibelly, 2026.</p>

### Análise

**Cibelly:** o maior cuidado na modelagem foi manter o nível conceitual sem descer a detalhes de implementação (getters/setters, tipos concretos de coleção, camadas de persistência), já que o [Diagrama de Pacotes](DiagramaPacotes.md) já reserva esse detalhamento para a camada `domain/entities`. Também precisei decidir onde extrair uma superclasse: `Personagem` concentra os atributos de combate compartilhados por `Jogador` e `Inimigo`, evitando duplicação sem introduzir uma hierarquia mais profunda do que os requisitos justificam.

Uma dificuldade foi conciliar duas versões dos requisitos funcionais: a lista inicial da própria SubEquipe_01 (ata de 11/09) e a lista RF01–RF19 do Subgrupo 03, adotada nos demais artefatos a partir da V2. Optei por manter `Combinacao` (fora de combate) e `Magia` (em combate) como classes distintas, pois cada conjunto de requisitos as descreve em momentos diferentes da jogatina — mas sinalizo essa escolha como uma inferência, não uma definição fechada da equipe.
...
|------------------|------------------|-----------------------------|
| `Combinacao` vs. `Magia`: a ata da SubEquipe_01 descreve combinação de elementos *fora* de combate gerando itens; os RF do Subgrupo 03 descrevem magias combinadas *durante* o combate. | Manter as duas classes separadas. | Pode ser um único conceito de "combinação" usado em dois momentos do jogo; recomenda-se validar com o Subgrupo 03. |
...
| O componente "Catálogo do Jogo" fornece definições de áreas, personagens, magias, itens e NPCs. | Não modelado como classe própria. | O próprio Diagrama de Componentes trata o Catálogo como serviço, não como classe de domínio; pode ser necessário representá-lo como repositório em versões futuras. |
...
<p align="center">Tabela 2: Incertezas assinaladas e decisões de modelagem. Fonte: comparação entre a ata de 11/09/2026 e os requisitos do Subgrupo 03, 2026.</p>
...
|:------:|:----:|-----------|-----------|---------|
| 1.0 | 17/09/2026 | Inserção do diagrama de classes. | Cibelly | - |
